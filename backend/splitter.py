...
class SplitWorker(QObject):
    ...
    def __init__(self, file_path, output_dir, original_stem=None, original_ext=".wav", models=None,
                 segment_size=5, overlap=0.25, shifts=1, guitar_split=False):
        super().__init__()
        ...
        self.shifts = shifts
        self.guitar_split = guitar_split
    ...
    def _run_model(self, model_name):
        ...
        if self.guitar_split:
            for stem in list(stems):
                if stem["name"] == "guitar":
                    self.statusChanged.emit("Splitting guitar into lead & rhythm...")
                    try:
                        new_stems = split_guitar(stem["path"])
                        stems.extend(new_stems)
                    except Exception as e:
                        self.statusChanged.emit(f"Guitar split failed: {e}")
