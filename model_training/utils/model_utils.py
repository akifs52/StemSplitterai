...
def get_hann_window(window_size: int) -> torch.Tensor:
    return torch.hann_window(window_size)


def wiener_refinement(mix: np.ndarray, lead: np.ndarray, rhythm: np.ndarray,
                       alpha: float = 0.5, n_fft: int = 2048,
                       hop_length: int = 512) -> tuple:
    import librosa
    S_mix = np.abs(librosa.stft(mix, n_fft=n_fft, hop_length=hop_length)) ** 2
    S_lead = np.abs(librosa.stft(lead, n_fft=n_fft, hop_length=hop_length)) ** 2
    S_rhythm = np.abs(librosa.stft(rhythm, n_fft=n_fft, hop_length=hop_length)) ** 2
    residual = np.maximum(S_mix - S_lead - S_rhythm, 0.0)
    eps = 1e-8
    W_lead = (S_lead + alpha * residual) / (S_lead + S_rhythm + residual + eps)
    W_rhythm = (S_rhythm + alpha * residual) / (S_lead + S_rhythm + residual + eps)
    S_mix_c = librosa.stft(mix, n_fft=n_fft, hop_length=hop_length)
    lead_refined = librosa.istft(S_mix_c * W_lead, hop_length=hop_length, length=mix.shape[-1])
    rhythm_refined = librosa.istft(S_mix_c * W_rhythm, hop_length=hop_length, length=mix.shape[-1])
    return lead_refined, rhythm_refined
